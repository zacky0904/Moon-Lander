...
public class DescentStage extends GameObject {
	...
	private BufferedImage descentStage;
	...
	private boolean hideAscentStage = false;
	...
	public DescentStage() {
		...
		try {
			//load image
			descentStage = ImageIO.read(getClass().getResourceAsStream("/Tile/Apollo Lunar Lander.png")).getSubimage(0, 82, 40, 20);
	...
	private void buildImage() {
		image = new BufferedImage(56, 44, BufferedImage.TYPE_INT_ARGB) ;
		Graphics2D imgBuffer = image.createGraphics();
		if(!hideAscentStage)
			imgBuffer.drawImage(ascentStage, 12, 0, null);
		imgBuffer.drawImage(descentStage, 8, 20, null);
		
		if(FLAME_ANIMATION_FLAG) {
			imgBuffer.drawImage(FLAME.getImage(), 24, 36, null);
		}
	...
	public void hideAscentStage(boolean flag) {
		hideAscentStage = flag;
	}
